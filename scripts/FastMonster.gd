...
func move_sorter(a, b) -> bool:
	var delta_x_a = a.x - player_pos.x
	var delta_y_a = a.y - player_pos.y
	var delta_x_b = b.x - player_pos.x
	var delta_y_b = b.y - player_pos.y
...
func _prepare(idx: int) -> void:
	...
	if abs(delta_x) <= 1 and abs(delta_y) <= 1:
		...
		if game_state.prepare_monster_attack(idx, attacktiles):
			Utility.create_monster_attacks(get_parent(), Attack, self.idx, self.game_state, attacktiles)
			return
